...
const navItems = ["index", "work", "story", "recognition", "contact"];
const universes = [
  ...
  return (
    <main className="portfolio-shell">
      ...
      <header className="topbar">
        ...
        <nav className={menuOpen ? "nav open" : "nav"}>{navItems.map((item, i) => <button key={item} className={active === item ? "active" : ""} onClick={() => jump(item)}><span>0{i + 1}</span>{item}</button>)}</nav>
      </header>
      ...
      <section id="index" className="hero section-pad">
        <div className="hero-copy reveal">
          ...
          <div className="hero-actions"><button className="button-primary" onClick={() => jump("work")}>ENTER THE WORK <span>↗</span></button><button className="text-link" onClick={() => jump("story")}>READ THE STORY <span>↓</span></button></div>
        </div>
      ...
      <section className="feature-section section-pad"><div className="feature-frame"><div className="feature-media"><div className="media-grid" /><span className="media-stamp">CASE STUDY / 001</span><span className="media-placeholder">MEDIA<br />ARCHIVE<br />LOADING</span></div><div className="feature-copy"><p className="eyebrow">FEATURED PRODUCT</p><h2>Carevo<br /><em>healthcare from home.</em></h2><p>A virtual hospital and healthcare ecosystem connecting patients, practitioners, pharmacy and laboratory services remotely.</p><div className="stat-row"><div><strong>700+</strong><span>practitioners</span></div><div><strong>3,000+</strong><span>patients</span></div><div><strong>01</strong><span>ecosystem</span></div></div><button className="button-primary" onClick={() => jump("contact")}>VIEW CASE STUDY <span>↗</span></button></div></div></section>
